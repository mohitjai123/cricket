  ...
  return (
    ...
    <header className={`flex  z-50 justify-between duration-300 text-black bg-white shadow-lg sticky top-0 h-20 items-center lg:px-16 px-4
     py-5`}>
        ...
        <div className='flex gap-4'>
            <button onClick={()=>setLoginPoup(true)} className={` border shadow-lg bg-secondary flex items-center gap-2 rounded-full h-fit py-2 my-auto px-2 text-sm lg:px-8 font-bold duration-500 hover:opacity-50 text-white `}> <span>Login</span> </button>
            <Link to="/registration-now" className={` border shadow-lg animate-btn flex items-center gap-2 rounded-full h-fit py-2 my-auto px-2 text-sm  lg:px-8 font-bold duration-500 hover:opacity-50 text-white `}> <span>Register Now</span> </Link>
        </div> : 
