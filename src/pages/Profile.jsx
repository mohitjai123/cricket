...
function Profile() {
    ...
    async function handleFile(e) {
        ...
        form.append("folder", "users")
          ...
          console.log(res);
          setFormData((prev)=>({...prev, pofile_img:`https://mjpl.co.in/users/${res.data.file}`}))
        } catch (error) {
          console.log(error);
                    ...
                    <div onSubmit={hanldeSubmit} className='grid' action="">
                       ...
                       <h4 className='bg-primary py-3 font-semibold text-white'>
                        ...
                        <form onSubmit={hanldeSubmit} className='grid px-5 mt-8 gap-x-10 gap-y-5 lg:grid-cols-3 md:grid-cols-2 grid-cols-1'>
                            ...
                            <LabelInputBox disabled={active!=1} onChange={handleFile} label="Profile Picture" placeholder='Please select date' name='pofile_img' type='file' />
...
{active == 1 && 
                       ...
                       <form onSubmit={hanldeSubmit} className='grid px-5 mt-8 gap-x-10 gap-y-5 lg:grid-cols-3 md:grid-cols-2 grid-cols-1'>
                       <LabelInputBox disabled={active!=2} onChange={(e) => e.target.value.length < 11 ? handleChange(e) : null} value={formdata.mobile_number} label="Mobile Number" type='number' placeholder='Please type your mobile number' name='mobile_number' />
