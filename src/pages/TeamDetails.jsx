...
function TeamDetails() {
    ...
    return (
        <section>
            <BreadCamp img={API_URL+"other-images/IMG_4955.JPG"} name="Teams" second={activeTeam?.name} />
           <div className='lg:px-24 py-14 p-4' style={{backgroundColor:activeTeam.logoColor}}>
          ...
          <div className='bg-grayScale lg:px-24 py-14 p-4'>
...
const TeamCards = ({item, isActive=false}) => {
    ...
    return (<Link className={`bg-primary rounded-lg w-32 shadow hover:shadow-lg relative border duration-300 ${isActive ? "border-white" : "border-gray-800"} hover:border-white`} to={"/teams/"+item.id}>
        ...
        <img className='h-28 mx-auto' src={item.logo || "https://cdn4.vectorstock.com/i/1000x1000/88/08/golden-monogram-logo-initial-letters-bg-vector-45018808.jpg"} alt="" />
    </Link>)
}

const TeamLogoCard =({item,idx})=>{

    return <div className='lg:flex gap-5'>
       <div className='relative mx-auto h-60 grid place-items-center w-60 z-10'>
       <img src={bgImg} className='absolute -z-10 top-0' alt="" />
       <img className='h-32 bg-white rounded-full mb-12' src={item.logo} alt="" />
       </div>
       <div className='h-fit my-auto'>
        <h3 className='text-white text-3xl text-center font-semibold'>{item.name}</h3>
...
const MememberCard = ({item})=>{
    return <Link to={"/players/ms-thoni"} className='border border-gray h-fit lg:w-fit bg-white'>
        <div className='bg-white border'>
        <img className='lg:h-60' src={item.profile_pic || "https://documents.iplt20.com/ipl/IPLHeadshot2024/57.png"} alt="" />
    <hr />
